  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        {/* Results Section */}
        {/* Results Section - Always visible effectively, but empty if no result */}
        <div className={`transition-all duration-300 ${result ? 'opacity-100' : 'opacity-50 blur-sm pointer-events-none'}`}>
          {result ? (
            result.isError ? (
            ...
            /* Placeholder empty state to keep layout stable if desired, or just render nothing but keep the container */
            <div className="bg-slate-800/20 border border-slate-700/50 rounded-xl p-6 min-h-[160px] flex items-center justify-center">
              <span className="text-slate-600 text-sm italic">Enter an expression to see results...</span>
            </div>
          )}
        </div>
      </div>
